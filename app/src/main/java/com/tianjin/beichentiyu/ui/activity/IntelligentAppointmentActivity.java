...
public class IntelligentAppointmentActivity extends BaseActivity {
    ...
    public static void toActivity(Context context, IntellectMsgBean.FieldBean fieldBean) {
        Intent intent = new Intent(context, IntelligentAppointmentActivity.class);
...
//    }
    ...
    private void getAllMemLogAppointmentIntellect(String fId, String date){
        if(falList == null){
            ToastUtil.showToast(R.string.request_failure);
            return;
        }
        ApiManager.getBusinessService().getAllMemLogAppointmentIntellect(AccountManager.getInstance().getAccount(), AccountManager.getInstance().getNonstr(), fId, date)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
