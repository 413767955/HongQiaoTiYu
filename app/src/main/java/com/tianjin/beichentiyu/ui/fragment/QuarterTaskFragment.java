...
public class QuarterTaskFragment extends BaseFragment {
    ...
    public static QuarterTaskFragment newInstance(int type){
        QuarterTaskFragment fragment = new QuarterTaskFragment();
    ...
    protected void initData() {
        ...
        if(bundle != null){
            type = bundle.getInt(TYPE_KEY,QUARTER_TYPE);
        }
    }
    ...
    private void initDataRv(){
        ...
        mDataAdapter.setItemClickListner(position -> {
            MaintenanceTaskInfoActivity.toActivity(getContext(),mDataList.get(position).getLog_id(),
                    MaintenanceTaskInfoActivity.INSPECTION_CODE);
        });
