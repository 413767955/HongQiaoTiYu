 ...
 */
public abstract class HeaderRecyclerAndFooterWrapperAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (mHeaderDatas.get(viewType) != null) {//不为空，说明是headerview
            return new BaseViewHolder(LayoutInflater.from(parent.getContext()).inflate(mHeaderDatas.get(viewType).keyAt(0),parent,false));
        } /*else if (mFooterViews.get(viewType) != null) {//不为空，说明是footerview
            return new BaseViewHolder(LayoutInflater.from(parent.getContext()).inflate(mFooterViews.get(viewType),parent,false));
        }*/
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        if (isHeaderViewPos(position)) {
            ...
            onBindHeaderHolder( holder, position, layoutId, mHeaderDatas.get(getItemViewType(position)).get(layoutId));
            return;
